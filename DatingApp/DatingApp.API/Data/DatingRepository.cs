...
namespace DatingApp.API.Data
{
  public class DatingRepository : IDatingRepository
  {
    ...
    public async Task<PagedList<User>> GetUsers(UserParams userParams)
    {
      ...
      return await PagedList<User>.CreateAsync(users, userParams.PageIndex, userParams.PageSize);
    }
    ...
    public async Task<PagedList<Message>> GetMessagesForUser(MessageParams messageParams)
    {
      ...
      {
        case "Inbox":
          messages = messages.Where(u => u.Recipient.Id == messageParams.UserId && u.RecipientDeleted == false);
          break;
        case "Outbox":
          messages = messages.Where(u => u.Sender.Id == messageParams.UserId && u.SenderDeleted == false);
          break;
        default:
          messages = messages.Where(u => u.Recipient.Id == messageParams.UserId && u.RecipientDeleted == false && u.IsRead == false);
          break;
      }

      messages = messages.OrderByDescending(m => m.MessageSent);
      return await PagedList<Message>.CreateAsync(messages, messageParams.PageIndex, messageParams.PageSize);
    }
